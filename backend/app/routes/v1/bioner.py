...
def _filter_invalid_date_entities(
    ...
    """
    Keep only date entities that pass strict date parsing.
    Non-date entities are always preserved.
    """
    filtered: list[dict[str, Any]] = []
    for entity in entities:
        label = entity.get("label")
        text = entity.get("text")
        if _is_date_label(label, date_label) and _parse_date_value(str(text or "")) is None:
            continue
